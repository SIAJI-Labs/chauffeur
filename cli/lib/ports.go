...
func (pm *PortManager) GetPortFromPrompt(service string, conflictPort int, suggestions []int) (int, error) {
	...
	logger.Prompt("[ ports ] Your choice [s/c/a]:", "Enter s, c, or a")
	var response string
	fmt.Scanln(&response)
	...
	case "s", "suggest":
		...
		logger.Prompt(fmt.Sprintf("[ ports ] Choose port number (1-%d):", len(suggestions)), "Enter a number")
		var choice string
		fmt.Scanln(&choice)
	...
	case "c", "custom":
		...
		logger.Prompt("[ ports ] Port:", "Enter a number between 1-65535")
