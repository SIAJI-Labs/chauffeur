...
func TestRemove_MultipleServices(t *testing.T) {
	...
	// Create mock nginx installation
	nginxBinary := filepath.Join(workspaceDir, "nginx", "sbin", "nginx")
	if err := os.MkdirAll(filepath.Dir(nginxBinary), 0755); err != nil {
		t.Fatalf("Failed to create nginx directory: %v", err)
	}
	if err := os.WriteFile(nginxBinary, []byte("#!/bin/bash\necho nginx"), 0755); err != nil {
		t.Fatalf("Failed to create fake nginx binary: %v", err)
	...
	// Create mock Composer installation
	composerBinary := filepath.Join(workspaceDir, "bin", "composer")
	if err := os.MkdirAll(filepath.Dir(composerBinary), 0755); err != nil {
		t.Fatalf("Failed to create composer directory: %v", err)
	}
	if err := os.WriteFile(composerBinary, []byte("#!/bin/bash\necho composer"), 0755); err != nil {
		t.Fatalf("Failed to create fake composer binary: %v", err)
	}
	composerDir := filepath.Join(workspaceDir, "composer")
	if err := os.MkdirAll(composerDir, 0755); err != nil {
		t.Fatalf("Failed to create composer data directory: %v", err)
	}

	// Remove both services with --force
	output := captureOutput(func() error {
		return commands.RunRemove([]string{"nginx", "composer", "--force"})
	})
	...
	if !strings.Contains(output, "Removed Composer") {
		t.Errorf("Expected success message about removing Composer, got: %s", output)
	}

	// Verify nginx directory was removed
	if _, err := os.Stat(filepath.Join(workspaceDir, "nginx")); !os.IsNotExist(err) {
		t.Error("Expected nginx directory to be removed")
	}

	// Verify composer binary and directory were removed
	if _, err := os.Stat(composerBinary); !os.IsNotExist(err) {
		t.Error("Expected composer binary to be removed")
	}
	if _, err := os.Stat(composerDir); !os.IsNotExist(err) {
		t.Error("Expected composer directory to be removed")
	}
}
...
func TestRemove_PHPVersionWithOtherServices(t *testing.T) {
	...
	if err := os.WriteFile(composerBinary, []byte("#!/bin/bash"), 0755); err != nil {
		t.Fatalf("Failed to create fake composer binary: %v", err)
	}
	composerDir := filepath.Join(workspaceDir, "composer")
	if err := os.MkdirAll(composerDir, 0755); err != nil {
	...
	output := captureOutput(func() error {
		return commands.RunRemove([]string{"php", "8.3", "composer", "--force"})
	})
	...
	if _, err := os.Stat(composerDir); !os.IsNotExist(err) {
		t.Error("Expected composer data directory to be removed")
	}
